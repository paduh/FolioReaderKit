...
open class FolioReaderCenter: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    override open func viewDidLoad() {
        ...
        enableScrollBetweenChapters(scrollEnabled: false)
    ...
    func updateCurrentPage(_ page: FolioReaderPage? = nil, completion: (() -> Void)? = nil) {
        ...
        let _ = currentPage.webView?.js("getReadingTime()", completion: { readTime in
            if let readTime = readTime {
                self.pageIndicatorView?.totalMinutes = Int(readTime)!
            } else {
    ...
    @objc func shareChapter(_ sender: UIBarButtonItem) {
        ...
        if let chapterText = currentPage.webView?.js("getBodyText()", completion: { chapterTxt in
            let htmlText = chapterTxt?.replacingOccurrences(of: "[\\n\\r]+", with: "<br />", options: .regularExpression)
